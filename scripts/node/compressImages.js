...
function detectChangedImages(baseDir) {
...
function processChangedImages(changedImages) {
    ...
    for (const [dir, imagePaths] of changedImages) {
        ...
        try {
            execSync(`python3 ${pythonScript} ${dir}`, { stdio: 'inherit' });
...
function forceProcessAllImages(baseDir) {
    ...
    clearCache();
    return checkAndProcessImages(baseDir);
}

// main function - new strategy for detecting and processing only changed images
function checkAndProcessImages(baseDir) {
    const changedImages = detectChangedImages(baseDir);
    return processChangedImages(changedImages);
}
...
if (require.main === module) {
    ...
    if (args.includes('--force') || args.includes('-f')) {
        ...
        if (processed) {
            ...
            console.log(`no compression needed - all images are up to date`);
        }
    }
}
